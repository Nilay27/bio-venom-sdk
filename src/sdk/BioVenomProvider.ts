...
export class BioVenomProvider {
  ...
  public getAnyWalletContract(address: string) {
    ...
    const contract = new this.provider.Contract(
      this.walletAbi,
      contractAddress,
    );
    return contract;
  ...
  public async preCalculateAddress(publicKey: any): Promise<string> {
    ...
    const preCalculatedAddress =
      await this.bioVenomDeployerInstance.calcWalletAddress(
        publicKey[0],
        publicKey[1],
      );
  ...
  public async deployWalletContract(publicKey: any): Promise<string> {
    ...
    try {
      const walletAddress =
        await this.bioVenomDeployerInstance.deployWalletContract(
          publicKey[0],
          publicKey[1],
        );
  ...
  public async createUnsignedUserOp(
    encodedPayload: any,
    value: number = 0,
  ): Promise<any> {
    /**
  ...
  public async encodeSignatureParams(
    rs: any,
    x1: any,
    y1: any,
    x2: any,
  ...
  public async encodeUserOperation(
    encodedSignature: any,
    encodedPayload: any,
    value: number = 0,
  ): Promise<any> {
    const nonce = (
      await this.bioVenomDeployerInstance.runGetMethod(
        'getNonce',
        this.walletAddress,
      )
    ).value0;
  ...
  public async signTvmCellUserOp(
    unsignedUserOp: any,
    encodedId: any,
    pubkey: any,
  ): Promise<any> {
    const { rs, x1, y1, x2, y2 } = await this.signer.sign(
      unsignedUserOp,
      encodedId,
      pubkey,
    );
    const encodedSignature = await this.encodeSignatureParams(
      rs,
      x1,
      y1,
      x2,
    ...
    const signedTVMCellUserOp = await this.encodeUserOperation(
      encodedSignature,
      this.unsignedUserOp._payloadOp,
      0,
    );
