...
export const createCredential = async (username: string): Promise<Credential | null> => {
  ...
  let authenticationSupport: 'platform' | 'cross-platform' = isPlatformSupported ? 'platform' : 'cross-platform';
...
export const getCredential = async (credentialId: string, challenge: string): Promise<Credential | null> => {
  try {
...
export const getPublicKey = async (attestationObject: Buffer) => {
  const authData = cbor.decode(attestationObject, undefined, undefined).authData as Uint8Array;
